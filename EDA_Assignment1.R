...
# Download file using the url listed here below and unzip
...
# Head of table to ensure format, then read it in
...
# Formatting dates and times appropriately. This is over a 48 hr period in Feb 2007
hpc$Date<-as.Date(hpc$Date, format = "%d/%m/%Y")
hpc$Time<-strptime(paste(hpc$Date,hpc$Time),"%F %T")

# Subset specific date range for that two day window of interest
hpc<-subset(hpc,hpc$Date %in% as.Date(c("2007-02-01","2007-02-02")))

# Now lets get into the plotting. Figure size has been assigned accordingly
# Used dev.off() function to be able to save the plots as a png
